\chapter{Software}\label{chap:software}
    \section{Environment Preparation}\label{sec:env preperation}
        ...
        The sources for the software API\nomenclature[A]{API}{Application Programming Interface} \textit{Handel} however are provided on their public repository on \textsc{GitHub}~\cite{Software.HandelRelease.2023} and are forked by the author of this work~\cite{Software.XraythesisHandel.2023}.
        As stated in \textit{Handel}'s API documentation \textquote{Handel is a framework of several libraries that interact to create the interface Handel provides}~\cite{Manual.HandelAPIManual.Xiang}.
        ...
        \subsection{Building Handel's API Libraries}\label{sec:software preperations}
            ...
            \textit{Handel} is meant to be built using \textit{SCons}~--~\textquote{a computer software build tool} which in turn is written in python.
            ...
            With a working conda install set up, a virtual environment is created, installing a specific Python version~\num{3.9} and activated afterwards following \cref{lst:creating conda env}.
    ...
    \section{Detector Interface Class}\label{sec:detector interface}
        ...
        All of \textit{Handel}'s top-level functions are named camel case and begin with ``xia''~--~they shall be hereafter called xia-functions.
        ...
        Any call to an xia-function takes either a value of type \texttt{c\_int()} to specify the detector channel (in this case always \texttt{0} as there is only one detector present), an ascii-encoded byte-stream representing an instruction and a reference to a predefined \textit{ctype}-variable either holding a value to set or to receive a value retrieved by the particular xia-function.
        \texttt{XMagix}'s method \texttt{stringToBytes()} takes a \texttt{str}-object as an input and returns a byte-stream.
        ...
        The constructor of \texttt{XMagix}, as shown in~\cref{lst:xmagix constructor}, besides pre-setting some internal state variables, loads the shared object library from the path passed to it during object creation.
        
        \subsection{init()}\label{sec:init}
            With the detector came a \texttt{*.ini}-file holding values and variables pre-defined by the manufacturer and specific to that particular device.
        ...
        \subsection{startSystem()}\label{sec:startSystem}
            After initializing the system using the method described in~\cref{sec:init}, the system can be started up using \texttt{<class object name>.startSystem()}.
        ...
        \subsection{setAcquisitionValues()}\label{sec:setAcquisitionValues}
            ...
            The name will be checked against a list of allowed names specified in Appendix A of \textit{Handel's programmer guide}~\cite{Manual.HandelProgrammersGuideMicroDXP.Xiang} and the value stored in the volatile memory if the check returns no error.\par\medskip

            This way the user is able to quickly vary different acquisition values like \texttt{gain\_trim} or any of the filter thresholds.\par\medskip
        ...
        \subsection{getAcquisitionValues()}\label{sec:getAcquisitionValues}
            ...
            The method returns a dictionary where the key-value pair reflects the specified name(s) and value(s).\par\medskip
        ...
        \subsection{fixedRealtimeRun()}\label{sec:fixedRealtimeRun}
            ...
            The optional argument \texttt{clearMca}, if set to \texttt{True} (default), clears the internal multichannel analyzer's bins prior starting the run.
            During the data acquisition, alongside the elapsed and total time, basic statistics like output count rate (OCR)\nomenclature[A]{OCR}{Output Count Rate} and the sum of all output events during the run (EIR)\nomenclature[A]{EIR}{Events In Run} are printed to the console in intervals of \qty{1}{\second}.\par\medskip
        ...
        \subsection{pullMcaData()}\label{sec:pullMcaData}
            After a successful run the data is stores in the \textit{microDXP}'s multichannel analyzer (MCA)\nomenclature[A]{MCA}{MultiChannel Analyzer}.
            \texttt{pullMcaData()} writes the MCA's content to a pre-defined array of length \texttt{nmca} and type \texttt{c\_ulong} and returns it as a \textit{numpy} array without clearing the MCA's content.
    ...
    \section{X-Ray Source Interface Class}\label{sec:xrs interface class}
        ...
        \end{lstlisting}
        \Cref{lst:SPI0 and pigpio} utilizes any \textit{Raspberry Pi OS's} built-in tool \texttt{raspi-config} in non-interactive mode to quickly activate the SPI on bus 0.
        ...
        \subsection{Initializing the SPI Bus}\label{sec:init spi bus}
            ...
            Clock polarity defines the idle state of the clock, thus, the clock's logic level while \(\overline{\texttt{CS}}\)\footnote{\texttt{CS}\nomenclature[A]{CS}{Chip Select} i.e. Chip Select is synonym with \texttt{CE}\nomenclature[A]{CE}{Chip Enable} i.e. Chip Enable.} is logic high.
            ...
            \lstinputlisting[style=mypython, firstnumber=5,
            caption={[\texttt{Tube} class's constructor]\texttt{Tube} class's constructor.},
            label={lst:Tube constructor}, firstline=5, lastline=28]{software/thetube.py}

            To let the ADC begin sampling the analog signal at its input, it expects a single start bit and three configuration bits after which it starts to clock back one \texttt{NULL} bit followed by its sampling result of 12 bits.
            Aligned with the clock edge transitioning from idle, the receive-buffer starts to fill with random bits immediately after \(\overline{\texttt{CS}}\) is pulled low.
            Following the application information of the \textit{MCP3202} sending a total of 3 bytes consisting of seven leading zeros, the start bit, three configuration bits and 13 \texttt{Don't Care} bits, the receive-buffer will fill with three bytes as well (see \cref{fig:adc communication protocol}), with bit 0 of the last received byte being the least significant bit (LSB) and bit 3 of the second byte the most significant bit (MSB) of the conversion result.
            \begin{figure}[h]
                \centering
                \includesvg[width=.8\textwidth]{electronics/datasheets/mcp3202_protocol}
                \caption[ADC communication protocol]{ADC communication protocol~\cite{Manual.ADC.MCP3202}.}%
            ...
            \subsection{enable()}\label{sec:tube enable}
                The method \texttt{enable()} is a shortcut to set \texttt{enpin} high, enabling the acceleration voltage, filament current and setting an internal state variable accordingly.
                The user can use this method and \texttt{disable()} in \cref{sec:tube disable} to programmatically enable and disable the XRS prior and after each data acquisition run.\par\medskip

                {\bfseries Warning: this enables the x-ray source if \texttt{HV Control} (\texttt{setHV()}) and \texttt{Current Control} (\texttt{setI()}) values are set \(> 0\)!}\par\medskip
            ...
            \subsection{composeBytesDac()}\label{sec:composebytesdac}
                Tube class's internal helper function \texttt{composeBytesDac()} shown in \cref{lst:Tube method composebytesdac} is not supposed to be invoked by user-level methods.
                ...
                The application in this work demands no high-speed output, thus, to ensure better signal integrity by reducing the stability requirements of the reference voltage, fast mode is set to False~\cite{Manual.ADC.MCP3202}.
                ...
                \lstinputlisting[style=mypython, firstnumber=42,
                caption={[\texttt{Tube} method \texttt{composeBytesDac()}]\texttt{Tube} method \texttt{composeBytesDac().}},
                label={lst:Tube method composebytesdac}, firstline=42, lastline=66]{software/thetube.py}

            \subsection{setHV()}\label{sec:setHV}
                \texttt{setHV()} is a method to translate a desired acceleration voltage in \unit{\kev} to a \qty{12}{\bit} integer value to pass to the DAC's channel 0.
                ...
                In any case, the actual set value for the acceleration voltage is stored in a class variable \texttt{hv} for later calculations of the maximum allowed filament current.\par\medskip
            ...
            \subsection{setI()}\label{sec:setI}
                \texttt{setI()} is a method to translate a desired filament current in \unit{\micro\ampere} to a \qty{12}{\bit} integer value to pass to the DAC's channel 1.
                The principle of mapping the desired current to an input value is basically the same as discussed in~\cref{sec:setHV}, but to ensure operating the XRS inside its specification limits the value of argument \(\texttt{i}\) undergoes an extra conditioning step, limiting the output to an appropriate value.
                ...
                The XRS is designed to operate at a maximum output power of \qty{12}{\watt}.
    ...
    \section{Motion System}\label{sec:motion system}
        ...
        Since reproducing that functionality is a non-trivial task and due to time limitations, the decision in favor of proven software was made despite it packing far more features than probably needed.
        The popular 3D-printer firmware \textit{Klipper}~\cite{Software.Klipper.2023} was chosen to generate the stepping pulses.
        
        Due to time limitations, no XMagix user interface is yet integration.
        To still be able to drive the motion system, a pre-made 3D-printing web interface \textit{Fluidd}~\cite{Software.FluiddTheKlipperUI} is used instead.
        ...
        In the case of the used \textit{Octopus v1.0}, the microcontroller architecture \texttt{LPC176x} and the processor model \texttt{LPC1769} must be selected.
    ...
    homing_positive_dir: false
        ...
        Since each motor is driven by a dedicated \textit{TMC2209} stepper driver, each section defining a physical axis is accompanied by a special \texttt{tmc2209} section (see~\cref{lst:stepper tmc section} for reference).
        ...
        Additionally, the stepper drivers feature StealthChop2~--~a \textquote{voltage-chopper based principle} to guarantee \textquote{that the motor is absolutely quiet in standstill and in slow motion}~\cite{Manual.TMC2209Datasheet} in trade for more heat generated in the motor windings while reducing torque at any given speed compared to normal operation.
