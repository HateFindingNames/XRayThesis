\chapter{Software}
    ...
    serial: /dev/serial/by-id/usb-Klipper_stm32f446xx_440022001251303431333234-if00
        ...
        Hereafter, the sections \texttt{[stepper\_x]}, \texttt{[stepper\_y]} and \texttt{[stepper\_z]} need to be populated for \texttt{Klipper} service to start without error.
        ...
        % \textit{Klipper} uses a special configuration file \texttt{printer.cfg} located in \texttt{~/printer\_data/config}.
        % \begin{lstlisting}[style=mybash, caption={[]}, label={lst:}]
            
        % \end{lstlisting}
        % The way stepper motors are build, driving them demands pulsing their pole pairs in a certain
