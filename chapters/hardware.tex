\chapter{Hardware}\label{chap:hardware}
    ...
    \section{Mechanics}\label{sec:mechanics}
        ...
        \begin{figure}[!h]
            \centering
            \includesvg[width=.4\textwidth]{drawings/sketch_stages}%
        ...
        The rise of desktop 3D-printing and the growing communities around highly sophisticated open source/hardware machines has made components for high precision yet low cost motion systems in the low to lower-mid power range more available than ever.
        ...
        \subsection{Sample Stage}\label{sec:sample stage}
            ...
            \begin{figure}[!ht]
                \centering
                \includesvg[width=.7\textwidth]{drawings/X-MAGIX_full_sample}%
            ...
            \begin{figure}[!h]
                \centering
                \includesvg[width=.3\textwidth]{pictures/footage/stage_bearings}
        ...
        \subsection{Detector Stage}\label{sec:detector stage}
            ...
            \begin{figure}[!ht]
                \centering
                \includesvg[width=.7\textwidth]{drawings/X-MAGIX_full_detector}%
            ...
            \begin{figure}[!h]
                \centering
                \includesvg[height=.4\textheight]{drawings/detector_drive_-_exploded}
        ...
        \subsection{Base and X-Ray Source}
            \begin{figure}[!ht]
                \centering
                \includesvg[width=.6\textwidth]{drawings/source_exploded}
                \caption[short]{title}%
    ...
    \section{Electronics}\label{sec:electronics}
        ...
        Following the considerations above, a \qty{5}{\volt} \qty{25}{\watt} power supply unit (PSU) \nomenclature[A]{PSU}{Power Supply Unit} (\textit{RS-25-5}) and a \qty{24}{\volt} \qty{150}{\watt} PSU (\textit{LRS-150-24}) are installed.
        ...
        \subsection{Kinematics}\label{sec:kinematics}
            ...
            \Cref{fig:endstop pic} shows an example of one of the actual endstops used.
            ...
            \begin{figure}[h]
                \centering
                \includesvg[width=.9\textwidth]{electronics/pdf/octopus}
            ...
            Low-level processing of the endstop signaling and stepping pulse generation takes place on dedicated peripheral hardware connected to the SBC via USB.
            ...
            \Cref{fig:octopus connections} depicts a top view diagram of the \textit{Octopus'} PCB\footnote{Printed Circuit Board\nomenclature[A]{PCB}{Printed Circuit Board}} layout and highlights the relevant connectors and terminals~\cite{Manual.BigTreeTechOctopus}.
            ...
            A dedicated stepper driver of type \textit{TMC2209} placed beneath the respective connector for each motor.
            The \textit{TMC2209} features various modes of operation each of which can be manipulated dynamically during operation.
            The \textit{Octopus'} microcontroller (MCU)\nomenclature[A]{MCU}{MicroController Unit} communicates with the drivers via 1-wire UART\nomenclature[A]{UART}{Universal Asynchronous Receiver-Transmitter}, needing a special pin assignment for each driver.\par\medskip
        ...
        \subsection{Raspberry Pi to X-Ray Source Interface}\label{sec:xrs interface board}
            ...
            Located next to the USB-B port is a 2x5 pin connector allowing analog control of the device and monitoring of its state%
            \footnote{%
            ...
            Current Control and HV Control\nomenclature[A]{HV}{High Voltage} are set by manipulating the voltage levels on pins 3 and 5.
            The SBC only provides pseudo-analogous control via built-in hardware pulse width modulation (PWM)\nomenclature[A]{PWM}{Pulse Width Modulation} \textquote{available on GPIO12, GPIO13, GPIO18, GPIO19}~\cite{Manual.Documentation.RPF}.
            ...
            Due to time restrictions, a rather crude interface board was made, utilizing a two-channel digital-to-analog converter (DAC) for writing to the XRS and a two-channel analog-to-digital converter (ADC) for reading it.
            To generate analog signals, the component \textit{TLV5618A} was used.
            It integrates two separate output channels with \(n = \qty{12}{\bit}\) resolution, delivers peak output voltages ranging from \qtyrange{2.7}{5.5}{\volt} and is SPI\nomenclature[A]{SPI}{Serial Peripheral Interface} programmable~\cite{Manual.DAC.TLV5618A}.
            ...
            \end{equation}
            \nomenclature[L]{\(REF\)}{Reference Voltage\nomunit{\volt}}%
            \nomenclature[L]{\(n\)}{ADC/DAC bit resolution\nomunit{1}}%
            \nomenclature[L]{\(FS\)}{Full Scale range\nomunit{\volt}}%
            \begin{align}
                UNOB = \cint*{ U \cdot \frac{n - 1}{FS}}
                \label{eq:UNOB}
            \end{align}

            \Cref{eq:dac general function}, taken from the DAC's datasheet, specifies a full scale range (\(FS\)) of two times the reference voltage while the output voltage remains capped at \(V_{DD}- \qty{0.4}{\volt}\).
            ...
            \Cref{fig:transfer function DAC} illustrates those dead regions.

            \begin{figure}[h]
                \centering
                \includesvg[width=\textwidth]{software/pictures/dac_transfer_function}
            ...
            \end{equation}
            \nomenclature[L]{\(R_S\)}{Voltage references current limiting resistor\nomunit{\ohm}}%
            \nomenclature[L]{\(U_S\)}{Supply Voltage\nomunit{\volt}}%
            \nomenclature[L]{\(U_Z\)}{Voltage across the reference voltage diode\unit{\volt}}%
            \nomenclature[L]{\(I_Z\)}{Current through reference voltage diode\nomunit{\ampere}}%
            \nomenclature[L]{\(I_L\)}{Load current\nomunit{\ampere}}%
            ...
            \Cref{tab:adc dac performance summery} summarizes the theoretical performance data of the D/A and A/D converter circuits.
            \begin{table}[t]
                \centering
                ...
                \begin{tabular}{@{}lSSSS@{}}
                    ...
                    \midrule
                    \(UNOB_L\)&     232&            35&                 222&            0{\tablefootnote{Since the filament current can not be practically set below \qty{10}{\uA} the effective \(UNOB_L\) is the same as that of current control.}}\\
            ...
            % The resolution in terms of \unit{\keV\per\bit} and \unit{\uA\per\bit}
