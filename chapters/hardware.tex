\chapter{Hardware}\label{chap:hardware}
    ...
    \section{Electronics}\label{sec:electronics}
        ...
        \subsection{Kinematics}\label{sec:kinematics}
            ...
            \Cref{fig:octopus connections} depicts a top view diagram of the \textit{Octopus'} PCB layout and highlights the relevant connectors and terminals~\cite{Manual.BigTreeTechOctopus}.
            ...
            For each motor is a dedicated stepper driver of type \textit{TMC2209} placed beneath the respective connector.
        ...
        \subsection{Raspberry Pi to X-Ray Source Interface}\label{sec:xrs interface board}
            ...
            \Cref{eq:dac general function}, taken from the DAC's datasheet, specifies a full scale range (\(FS\)) of two times the reference voltage while the output voltage keeps capped at \(V_{DD}- \qty{0.4}{\volt}\).
            The difference of \qty{0.17}{\volt} is expected to be clipped off of the full scale output signal.\par\medskip

            % The D/A converter's resolution is given as

            % \begin{equation}
            %     Res_{DAC} = \frac{2U_{Ref}}{n - 1} \approx \qty{1.2}{\milli\volt\per\bit}
            %     \label{eq:DAC resolution}
            % \end{equation}
            ...
            \end{equation}
            \nomenclature[L]{\(R_S\)}{Voltage reference series resistor\nomunit{\ohm}}%
